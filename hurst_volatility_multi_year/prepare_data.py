...
date_re = re.compile(rf"^{re.escape(prefix)}(\d{{4}}_\d{{2}}_\d{{2}})\.csv$")
...
if remove_FOMC_days:
    fomc_dates_set = set(FOMC_dates)
    filtered_files: List[Tuple[str, str]] = []
    for fn, d in csv_files:
        if d in fomc_dates_set:
            print(f"date {d} removed because FOMC day")
            continue
        filtered_files.append((fn, d))
...
for fn, date_str in csv_files:
    ...
    try:
        ...
        if prices.size == 0:
            skipped_errors += 1
            print(f"date {date_str} removed because empty price data")
            continue
        ...
        if remove_trading_halts is not None and int(remove_trading_halts) >= 0:
            ...
            if max_run > int(remove_trading_halts):
                skipped_halts += 1
                print(
                    f"date {date_str} removed because trading halt length {max_run} exceeds {int(remove_trading_halts)}"
                )
                continue
        ...
        kept += 1
        print(f"date {date_str} successfully added")
    except Exception:
        skipped_errors += 1
        print(f"date {date_str} removed because load failed")
        continue
...
year_to_prices = {}
for prices, date_str in zip(daily_prices, daily_dates):
    year = date_str[:4]
    year_to_prices.setdefault(year, []).append(prices)

base_name, ext = os.path.splitext(output_data_name)
if not ext:
    ext = ".npy"

for year, prices_list in sorted(year_to_prices.items()):
    year_prices_array = np.array(prices_list, dtype=object)
    np.save(f"{base_name}_{year}{ext}", year_prices_array)
