...
increment_volatility_size = 60 # Integer
...
compute_confidence_interval = False  # Unused for this script, kept for API compatibility
...
output_plot_folder = "output"
output_plot_prefix = "volatility_autocorrelation"
...
def compute_autocorrelation(
    ...
    if return_counts:
        return autocorr / float(window) * 17, n_increments, truncated

    return autocorr / float(window) * 17


def save_autocorrelation_quantiles_plot(
    lags: np.ndarray,
    median: np.ndarray,
    q025: np.ndarray,
    q975: np.ndarray,
    output_path: str,
    increment_window: int,
) -> None:
    fig, ax = plt.subplots(figsize=(9, 5.5))
    ax.fill_between(lags, q025, q975, color="tab:blue", alpha=0.2, label="2.5%-97.5% quantiles")
    ax.plot(lags, median, color="tab:blue", linewidth=2.0, label="Median")
    ax.axhline(0.0, color="black", linewidth=1.0, alpha=0.6)
    ax.set_xlabel(f"Lag index (1 lag = {increment_window} volatility increments)")
    ax.set_ylabel("Autocorrelation")
    ax.set_title("Volatility increments autocorrelation")
    ax.grid(alpha=0.2)
    ax.legend()
    fig.tight_layout()
...
def _run_pipeline_from_array(
    ...
    """Run the volatility-increment autocorrelation pipeline on a price array."""
    del (
        hurst_min_value,
        hurst_max_value,
        hurst_step,
    ...
    ########## PART 1 -- Check all the parameters
    if N_autocorrelation is None or int(N_autocorrelation) <= 0:
        raise ValueError("Config error: N_autocorrelation must be a positive integer.")
    ...
    for prices in daily_prices:
        try:
            vsq, _, _ = compute_volatility_squared(
                prices,
                window_size=window,
    ...
    ########## PART 4 -- Remove intraday pattern and normalise
    ...
    if remove_pattern is not None:
        ...
        if rp == "multiplicative":
            stacked = np.vstack(daily_volatility_squared_list)
            ...
            raise ValueError(
                f"Invalid remove_pattern: {remove_pattern}. "
                "Expected None, 'multiplicative', or 'additive'."
            )

    ########## PART 5 -- Compute autocorrelation vectors
    series_len = daily_volatility_squared_list[0].shape[0]
    if series_len <= n_lags:
        raise ValueError(
            f"Config error: N_autocorrelation ({n_lags}) must be smaller than "
            f"the volatility series length ({series_len})."
        )
    ...
    for vsq in daily_volatility_squared_list:
        ...
        if autocorr.shape[0] != n_lags or not np.all(np.isfinite(autocorr)):
            continue
        lag0 = float(autocorr[0])
        if not np.isfinite(lag0) or np.isclose(lag0, 0.0):
            continue
        autocorr = autocorr / lag0
    ...
    if not daily_autocorr_vectors:
        print("No valid autocorrelation vectors could be computed.")
        return None

    daily_autocorr_matrix = np.vstack(daily_autocorr_vectors)
    lags = np.arange(n_lags, dtype=int)
    q025 = np.quantile(daily_autocorr_matrix, 0.025, axis=0)
    median = np.quantile(daily_autocorr_matrix, 0.50, axis=0)
    q975 = np.quantile(daily_autocorr_matrix, 0.975, axis=0)
    mean = np.mean(daily_autocorr_matrix, axis=0)
    ...
    if print_truncated_infos:
        ...
        print(
            ...
            f"(n_truncated_days: {n_truncated_days})"
        )
    ...
    if output_plot_path is not None:
        ...
        save_autocorrelation_quantiles_plot(
            lags=lags,
            median=median,
            q025=q025,
            q975=q975,
    ...
    median_table = ["lag\tmedian_autocorrelation"]
    median_table.extend(
        f"{int(lag)}\t{float(value):.10g}" for lag, value in zip(lags, median)
    )
    print("\n".join(median_table))

    mean_table = ["lag\tmean_autocorrelation"]
    mean_table.extend(
        f"{int(lag)}\t{float(value):.10g}" for lag, value in zip(lags, mean)
    )
    print("\n".join(mean_table))

    if output_plot_path is not None:
        output_csv_path = os.path.splitext(output_plot_path)[0] + ".csv"
        csv_header = ",".join(f"lag_{int(lag)}" for lag in lags)
        np.savetxt(
            output_csv_path,
            daily_autocorr_matrix,
            delimiter=",",
            header=csv_header,
            fmt="%.10g",
            comments="",
    ...
    return {
        ...
        "q025": q025,
        "q975": q975,
        "daily_autocorrelation_matrix": daily_autocorr_matrix,
    }
...
def run_pipeline(
    ...
    check_optimisation: Optional[Union[float, bool]] = None,
    plot_covariance_matrix: bool = False,
    output_plot_folder: str = "output",
    output_plot_prefix: str = "volatility_autocorrelation",
) -> Optional[
    Union[
        Dict[str, np.ndarray],
        Dict[Tuple[int, ...], Optional[Dict[str, np.ndarray]]],
    ]
]:
    ...
    plot_folder_path = os.path.join(base_dir, output_plot_folder)
    ...
    if N_consecutive_years <= 0:
        ...
        plot_path = os.path.join(
            plot_folder_path,
            f"{output_plot_prefix}_{years[0]}_{years[-1]}.png",
        )
        ...
        if result is not None:
            print(f"Saved autocorrelation quantile plot: {plot_path}")
        return result

    results: Dict[Tuple[int, ...], Optional[Dict[str, np.ndarray]]] = {}
    ...
    for i in range(0, len(years) - span + 1):
        ...
        plot_path = os.path.join(
            plot_folder_path,
            f"{output_plot_prefix}_{span_years[0]}_{span_years[-1]}.png",
        )
        result = _run_pipeline_from_array(
            X,
            price_truncation_mode=price_truncation_mode,
        ...
        if result is None:
            print(f"Span: {span_years[0]}-{span_years[-1]} autocorrelation=None")
            continue
        print(f"Span: {span_years[0]}-{span_years[-1]} saved plot: {plot_path}")
    return results
